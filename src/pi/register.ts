...
import { health, publish, subscribe } from "../client.js";

export type PiApi = {
  on(event: string, handler: (event: any, ctx: PiContext) => void | Promise<void>): void;
...
export type PiNotify = {
  publish<T>(topic: string, payload: T, opts?: { ctx?: PiContext; meta?: Record<string, unknown> }): Promise<PublishResult>;
...
function ctxMeta(ctx: PiContext | undefined): Record<string, unknown> | undefined {
...
export async function register(pi: PiApi, opts: RegisterOptions = {}): Promise<PiNotify> {
  ...
  const statusKey = typeof opts.statusKey === "string" && opts.statusKey.trim().length > 0 ? opts.statusKey : "pi-notify";
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (!ctx.hasUI) return;
    const ok = await transportHealth({ endpoints: opts.bus?.endpoints, timeoutMs: opts.bus?.timeoutMs });
  ...
  return {
    async publish(topic, payload, callOpts = {}) {
      ...
      return transportPublish(topic, payload, { endpoints: opts.bus?.endpoints, timeoutMs: opts.bus?.timeoutMs, meta });
    },
    async subscribe(prefix, handler) {
      return transportSubscribe(prefix, handler, { endpoints: opts.bus?.endpoints, timeoutMs: opts.bus?.timeoutMs });
    },
    async health() {
      return transportHealth({ endpoints: opts.bus?.endpoints, timeoutMs: opts.bus?.timeoutMs });
    },
  };
}
